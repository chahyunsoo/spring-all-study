...
public class JpaMemberRepository implements MemberRepository{
    private final EntityManager entityManager;  //JPA는 EntityManager로 동작을 한다.
    /*
    ...
    * */
    ...
    public Optional<Member> findById(Long id) {
        Member member = entityManager.find(Member.class, id); //조회할 타입이랑,식별자 p.k값 넣어주면 조회가 된다. select문 나간다.
    ...
    public List<Member> findAll() {
         ...
         */
        List<Member> result = entityManager.createQuery("select m from Member m", Member.class) //Member Entity자체를 Select
                .getResultList();
        return result;
    }
}
