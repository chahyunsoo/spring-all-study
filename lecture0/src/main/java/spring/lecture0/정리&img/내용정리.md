...
@Service도 마찬가지로 스프링이 뜰 때, 컨테이너에 @Servicer가 붙은 클래스를 등록해준다.
(@Repository도 동일함)

MemberController와 MemberService를 연결시켜줘야 한다면, @Autowired를 사용하면 된다.
```java
@Service
public class MemberService {
    private final MemberRepository memberRepository;
...
다시 정리하면, @Component가 있으면 스프링 빈으로 자동 등록됨.
